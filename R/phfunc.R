...
phfunc <- function(otu, taxa, sample, tree, functional_profile, pre_threshold, hcol = 'red', ttip = 3, tlab = 2, hlab = 4, llab = 10, alab = 4, offset = 0.3, width = 1, mini_abun, threshold, sample_name, sample_group, mini_tpm, threshold_tpm, both_core_col, common_core_col, func_core_col, non_core_col){
  ...
  cf_core_a <- cf_core %>% filter(fill == 'common core', f_fill == 'core')
  if (dim(cf_core_a)[1] > 0){cf_core_a$type <- 'both-core'}
  cf_core_b <- cf_core %>% filter(fill == 'common core', f_fill == 'non-core')
  if (dim(cf_core_b)[1] > 0){cf_core_b$type <- 'common-core'}
  cf_core_c <- cf_core %>% filter(fill == 'non-common core', f_fill == 'core')
  if (dim(cf_core_c)[1] > 0){cf_core_c$type <- 'functional-core'}
  cf_core_d <- cf_core %>% filter(fill == 'non-common core', f_fill == 'non-core')
  ...
  cols <- c('both-core' = both_core_col, 'common-core' = common_core_col, 'functional-core' = func_core_col,  'non-core' = non_core_col)
  ...
  p <- p1 + geom_tile(data = heatmap_coordinates_abun, aes(x, y, fill = value), color = "black") +
    ...
    theme(legend.text = element_text(size = llab, colour = "black"),
          legend.title = element_text(face = "bold", size = llab, colour = "black"),
          legend.position = "top")
  p
}
