...
dis_occ_loop <- function(otu, sample, min_abun, max_abun, abun_step, min_thre, max_thre, thre_step, sample_name, sample_group) {
  ...
  print('the black means NA, which means in this combination of minimal abundance and threshold, NO cores can be identified.')
...
function_core <- function(functional_profile, sample, mini_abun, threshold) {
  ...
  fun_tpm_sum$fill <- 'non-core'
  fun_tpm_sum$fill <- c(rep('core', times = lastCall), rep('non-core', times = dim(fun_tpm_sum)[1] - lastCall))
...
dis_func_occ <- function(functional_profile, map, mini_abun, threshold) {
  ...
  fun_occ_core <- fun_occ %>% filter(fill == 'core')
  fun_occ_noncore <- fun_occ %>% filter(fill == 'non-core')
