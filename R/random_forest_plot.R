...
# There must be sample_name and sample_group in map file.
...
random_forest_plot <- function(otu, sample, mini_abun, threshold, sample_name, sample_group){
  ...
  otu_rf <- otu_rf %>% filter(rank %in% BC_ranked_abun$otu[BC_ranked_abun$fill == 'core'])
  row.names(otu_rf) <- otu_rf$rank
  otu_rf <- otu_rf[, !grepl("rank", colnames(otu_rf))]
  otu_rf <- as.data.frame(t(otu_rf))
  otu_rf$sample_name <- row.names(otu_rf)
  
  map_sub <- sample %>% select(sample_name, sample_group) # sample_name, sample_group
  
  data_rf <- otu_rf %>% left_join(map_sub, by = sample_name) # 'sample_name'
  data_rf <- data_rf[, !grepl(sample_name, colnames(data_rf))] # 'sample_name'
  data_rf$sample_group <- as.factor(data_rf$sample_group) # sample_group
  ...
  rf <- randomForest(y=train$sample_group, x=train[,1:ncol(train)-1], proximity = TRUE) 
  ...
  p_core_train_acc <- confusionMatrix(p_core_train, train$sample_group)$overall[1]
  p_core_test <- predict(rf, test[,1:ncol(test)-1])
  p_core_test_acc <-  confusionMatrix(p_core_test, test$sample_group)$overall[1]
  ...
  p_noncore_train_acc <- confusionMatrix(p_noncore_train, train$sample_group)$overall[1]
  p_noncore_test <- predict(rf, test[,1:ncol(test)-1])
  p_noncore_test_acc <-  confusionMatrix(p_noncore_test, test$sample_group)$overall[1]
  ...
  train <- data_rf[ind == 1,]
  test <- data_rf[ind == 2,]
  rf <- randomForest(y=train$sample_group, x=train[,1:ncol(train)-1], proximity = TRUE) # sample_group
  p_all_train <- predict(rf, train[,1:ncol(train)-1])
  p_all_train_acc <- confusionMatrix(p_all_train, train$sample_group)$overall[1] # sample_group
  p_all_test <- predict(rf, test[,1:ncol(test)-1])
  p_all_test_acc <-  confusionMatrix(p_all_test, test$sample_group)$overall[1] # sample_group
