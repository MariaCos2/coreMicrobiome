...
phnetworks <- function(otu, taxa, sample, tree, propr = TRUE, sparcc = TRUE, cclasso = TRUE, pre_threshold, fdr_threshold, cor_threshold, permutation, nscore = 'degree', hcol = 'red', propr_col = 'darkcyan', sparcc_col = 'firebrick3', cclasso_col = 'goldenrod3', ttip = 3, tlab = 2, hlab = 4, llab = 4, offset = 0.3, width = 1, mini_abun, threshold, sample_name, sample_group){
  ...
  otu_relabun_plot = get_avg_rel_abun(otu, sample, otu_ranked, BC_ranked_abun, sample_name, sample_group)
  ...
  p2 <- p1 + geom_tile(data = heatmap_coordinates_abun, aes(x, y, fill = value), color = "black") +
    ...
    annotate("text", x = x_htitle1, y = y_title, label = "Avg rel abun", size = llab) +
    annotate("text", x = x_htitle2, y = y_title, label = "Avg occ", size = llab)
  ...
  p <- p2 + geom_point(data = points, aes(x = x, y = y, size = local_score)) + labs(size = "Node score") +
    ...
    ylim(-2, y_title + 1) +
    annotate("text", x = x_r + max(links$yma_ymi)*0.05*0.5*0.5*0.5, y = y_title, label = cor_m[1], size = llab) +
  ...
  if (length(cor_m) >= 2){
    ...
    p <- p + geom_point(data = points, aes(x = x, y = y, size = local_score)) + labs(size = "Node score") +
      ...
      xlim(0, x_r + max(links$yma_ymi)*0.05*0.5*0.5) +
      ylim(-2, y_title + 1) +
      annotate("text", x = x_r + max(links$yma_ymi)*0.05*0.5*0.5*0.5, y = y_title, label = cor_m[2], size = llab) +
      theme(legend.title = element_text(colour = "black", size = llab),
            legend.text = element_text(colour ="black", size = llab))
  }
  ...
  if (length(cor_m) == 3){
    ...
    p <- p + geom_point(data = points, aes(x = x, y = y, size = local_score)) + labs(size = "Node score") +
      ...
      xlim(0, x_r + max(links$yma_ymi)*0.05*0.5*0.5) +
      ylim(-2, y_title + 1) +
      annotate("text", x = x_r + max(links$yma_ymi)*0.05*0.5*0.5*0.5, y = y_title, label = cor_m[3], size = llab) +
      theme(legend.title = element_text(colour = "black", size = llab),
            legend.text = element_text(colour ="black", size = llab))
  }
