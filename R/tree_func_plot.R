...
##' @examples
##'  tree_func_plot(functional_profile, sample, tree, mini_tpm=1000, threshold=0.5, offset = 0.1, width = 0.5, core_col = 'deeppink4', noncore_col = 'darkseagreen3', low_col = 'white', high_col = 'lightcoral', hlab = 2, ttip = 2, tlab = 2)
##' @export

tree_func_plot <- function(functional_profile, sample, tree, mini_tpm, threshold, offset, width, core_col, noncore_col, low_col, high_col, hlab, ttip, tlab){
  ...
  h <- fun_rel_df
  pt2 <- gheatmap(p1, h, offset = offset, width = width, low = low_col, high = high_col, color = "black", colnames_angle = 90, hjust = 0, font.size = hlab, legend_title = "Avg TPM")
