...
tree_func_plot <- function(functional_profile, sample, tree, mini_tpm, threshold, offset, width, core_col, noncore_col, low_col, high_col, hlab, ttip, tlab, hangle, hoffset){
  ...
  row.names(fun_occ_df) <- names
  #fun_occ_df <- fun_occ_df[,-grep("genome",colnames(fun_occ_df))]
  h <- fun_occ_df
  pt1 <- gheatmap(p1, h, offset = offset, width = width, low = low_col, high = high_col, color = "black", colnames_angle = hangle, colnames_offset_y = hoffset, hjust = 0, font.size = hlab, legend_title = "Occupancy frequency")

  fun_rel_df <- data.frame(fun_rel)
  row.names(fun_rel_df) <- names
